...
export function validateRealImportRehearsal(input) {
  if (!["inspect", "resume-inspect", "apply"].includes(input.phase))
    refuse("invalid_phase");
  if (
    input.confirm !==
    `restored-data-accounts-import-reconciliation-${input.phase}`
  )
    refuse("confirmation_missing");
  for (const key of [
    "projectId",
    "environmentId",
    "restoredPostgresServiceId",
    "operatorServiceId",
    "sourceServiceId",
    "aegyoCommit",
  ])
  ...
  if (
    input.accountsBaseURL !== "https://account.aegyoarena.com" ||
    input.aegyoBaseURL !== input.accountsBaseURL
  )
    refuse("unexpected_accounts_issuer");
  for (const [key, value] of Object.entries({
    legacyDatabase: input.legacyDatabase,
    accountsDatabase: input.accountsDatabase,
    sourceRole: input.sourceRole,
    legacyOwnerRole: input.legacyOwnerRole,
    accountsRuntimeRole: input.accountsRuntimeRole,
  }))
    if (!name.test(value ?? "")) refuse(`invalid_${key}`);
  if (
    !/^aegyo_auth_rehearsal_[0-9]{8}_[a-z0-9]{6,16}$/.test(input.legacyDatabase)
  )
    refuse("legacy_database_not_new_rehearsal_target");
  if (
    !/^accounts_rehearsal_[0-9]{8}_[a-z0-9]{6,16}$/.test(input.accountsDatabase)
  )
    refuse("accounts_database_not_new_rehearsal_target");
  if (input.legacyDatabase === input.accountsDatabase)
    refuse("database_names_must_differ");
  if (!Number.isSafeInteger(input.expectedUsers) || input.expectedUsers < 1)
    refuse("invalid_expected_users");
  const requiredUrls =
    input.phase === "inspect"
      ? ["source", "legacyOwner"]
      : input.phase === "resume-inspect"
        ? ["legacyOwner"]
        : ["legacyReader", "legacyOwner", "accounts"];
  const presentUrls = Object.keys(input.urls ?? {}).filter(
    (key) => input.urls[key],
  );
  if (
    presentUrls.length !== requiredUrls.length ||
    requiredUrls.some((key) => !presentUrls.includes(key))
  )
    refuse("unexpected_phase_database_connections");
  if (input.phase === "apply") {
    if (!opaqueId.test(input.canarySourceUserId ?? ""))
      refuse("invalid_canary_source_user_id");
    if (!hex.test(input.approvedSnapshotDigest ?? ""))
      refuse("invalid_approved_snapshot_digest");
    if (!hex.test(input.deployedPepperDigest ?? ""))
      refuse("invalid_deployed_pepper_digest");
  }
  for (const [key, value] of Object.entries(input.urls ?? {})) {
    ...
    if (
      ...
      refuse(`${key}_must_be_query_free_private_railway_url`);
  }
  return true;
}
