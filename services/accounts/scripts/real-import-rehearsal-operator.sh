...
for n in SOURCE_DATABASE_URL SOURCE_DATABASE_NAME SOURCE_READ_ONLY_ROLE REHEARSAL_LEGACY_DATABASE_URL \
 REHEARSAL_LEGACY_DATABASE_NAME REHEARSAL_DATABASE_OWNER_ROLE REHEARSAL_POSTGRES_SERVICE_ID \
 ACCOUNTS_IMPORT_TARGET_DATABASE_URL REHEARSAL_ACCOUNTS_DATABASE_NAME ACCOUNTS_DATABASE_ROLE \
 SOURCE_DATABASE_CA_CERT REHEARSAL_DATABASE_CA_CERT ACCOUNTS_IMPORT_TARGET_DATABASE_CA_CERT \
 REHEARSAL_DATABASE_SERVER_SHA256 ACCOUNTS_IMPORT_TARGET_DATABASE_SERVER_SHA256 \
 ACCOUNTS_IMPORT_APPROVED_DIGEST ACCOUNTS_IMPORT_SOURCE_NAMESPACE ACCOUNTS_IMPORT_EXPECTED_COUNT \
 ACCOUNTS_REAL_EXPECTED_TABLES \
 ACCOUNTS_REAL_DEPLOYED_PEPPER_DIGEST ACCOUNTS_LEGACY_PEPPER ACCOUNTS_BASE_URL AEGYO_AUTH_BASE_URL \
 AEGYO_SOURCE_COMMIT RAILWAY_PROJECT_ID RAILWAY_ENVIRONMENT_ID RAILWAY_SERVICE_ID \
 SOURCE_DATABASE_SERVICE_ID \
 ACCOUNTS_REAL_CANARY_PASSWORD ACCOUNTS_REAL_CANARY_EMAIL ACCOUNTS_REAL_CANARY_SOURCE_USER_ID \
 ACCOUNTS_DATABASE_ROLE_PASSWORD BETTER_AUTH_SECRET; do required "$n"; done

work=/operator/accounts/.proof/real-import
...
TARGET_DATABASE_URL="$REHEARSAL_LEGACY_DATABASE_URL" \
...
export ACCOUNTS_IMPORT_SOURCE_DATABASE_URL="$REHEARSAL_LEGACY_DATABASE_URL"
...
export PGPASSWORD="$(node -e 'process.stdout.write(new URL(process.env.REHEARSAL_LEGACY_DATABASE_URL).password)')"
legacy_host="$(node -e 'process.stdout.write(new URL(process.env.REHEARSAL_LEGACY_DATABASE_URL).hostname)')"
printf '%s\n' "$REHEARSAL_DATABASE_CA_CERT" >"$work/legacy-ca.pem"
PGSSLMODE=verify-full PGSSLROOTCERT="$work/legacy-ca.pem" psql -X -qAt -h "$legacy_host" -U "$(node -e 'process.stdout.write(new URL(process.env.REHEARSAL_LEGACY_DATABASE_URL).username)')" -d "$REHEARSAL_LEGACY_DATABASE_NAME" -v ON_ERROR_STOP=1 \
 -c 'SELECT json_build_object('"'"'version'"'"',1,'"'"'users'"'"',coalesce(json_agg(json_build_object('"'"'id'"'"',id,'"'"'role'"'"',role,'"'"'linkedRecords'"'"','"'"'{}'"'"'::json) ORDER BY id),'"'"'[]'"'"'::json)) FROM "User"' >"$work/local-before.json" 2>"$work/aegyo.err" || fail local_snapshot_failed
unset PGPASSWORD
...
# Apply the additive Aegyo schema only to the restored clone.
export PGPASSWORD="$(node -e 'process.stdout.write(new URL(process.env.REHEARSAL_LEGACY_DATABASE_URL).password)')"
PGSSLMODE=verify-full PGSSLROOTCERT="$work/legacy-ca.pem" psql -X -q -h "$legacy_host" -U "$(node -e 'process.stdout.write(new URL(process.env.REHEARSAL_LEGACY_DATABASE_URL).username)')" -d "$REHEARSAL_LEGACY_DATABASE_NAME" -v ON_ERROR_STOP=1 -f /operator/aegyo/prisma/migrations/20260911200000_add_shared_auth/migration.sql >"$work/additive.log" 2>&1 || fail additive_schema_failed
unset PGPASSWORD

# Aegyo's pinned Prisma CLI travels through a loopback-only TLS bridge. The
# bridge verifies the restored server certificate and never exposes a port.
printf '%s\n' "$REHEARSAL_DATABASE_CA_CERT" >"$work/stunnel-ca.pem"
observed_pin="$(openssl s_client -starttls postgres -connect "$legacy_host:5432" -servername localhost -CAfile "$work/stunnel-ca.pem" </dev/null 2>"$work/tls-check.err" | openssl x509 -noout -fingerprint -sha256 2>>"$work/tls-check.err" | sed 's/^.*=//;s/://g' | tr 'A-F' 'a-f')"
[ "$observed_pin" = "$(printf '%s' "$REHEARSAL_DATABASE_SERVER_SHA256" | tr -d ':' | tr 'A-F' 'a-f')" ] || fail restored_database_certificate_pin_mismatch
cat >"$work/stunnel.conf" <<EOF
foreground = yes
client = yes
[postgres]
accept = 127.0.0.1:6543
connect = $legacy_host:5432
CAfile = $work/stunnel-ca.pem
verifyChain = yes
checkHost = localhost
EOF
stunnel "$work/stunnel.conf" >"$work/stunnel.log" 2>&1 & bridge=$!
trap 'kill ${bridge-} 2>/dev/null || true; code=$?; [ "$code" = 0 ] || echo operator_error=real_import_rehearsal_failed >&2' EXIT HUP INT TERM
sleep 1; kill -0 "$bridge" 2>/dev/null || fail verified_tls_bridge_failed
export AEGYO_MAPPING_DATABASE_URL="$(node -e 'const u=new URL(process.env.REHEARSAL_LEGACY_DATABASE_URL);u.hostname="127.0.0.1";u.port="6543";u.search="?sslmode=disable";process.stdout.write(u.href)')"
export AEGYO_MAPPING_DATABASE_NAME="$REHEARSAL_LEGACY_DATABASE_NAME"
export AEGYO_AUTH_BASE_URL
...
export PGPASSWORD="$(node -e 'process.stdout.write(new URL(process.env.REHEARSAL_LEGACY_DATABASE_URL).password)')"
PGSSLMODE=verify-full PGSSLROOTCERT="$work/legacy-ca.pem" psql -X -qAt -h "$legacy_host" -U "$(node -e 'process.stdout.write(new URL(process.env.REHEARSAL_LEGACY_DATABASE_URL).username)')" -d "$REHEARSAL_LEGACY_DATABASE_NAME" -v ON_ERROR_STOP=1 \
 -c 'SELECT json_build_object('"'"'version'"'"',1,'"'"'users'"'"',coalesce(json_agg(json_build_object('"'"'id'"'"',id,'"'"'role'"'"',role,'"'"'linkedRecords'"'"','"'"'{}'"'"'::json) ORDER BY id),'"'"'[]'"'"'::json)) FROM "User"' >"$work/local-after-actual.json" 2>>"$work/aegyo.err" || fail post_mapping_snapshot_failed
unset PGPASSWORD
