...
## Current implementation and staging

The independent Better Auth service includes EN/ES account pages, Mailjet sender support, database-backed rate limiting, atomic credential/session guards, a restricted database role, explicit owner migration, synthetic bootstrap, and authenticated provider-session checks. Arcade integrates OIDC code/PKCE behind `ARCADE_SHARED_AUTH_ENABLED`, using separate membership cookies. Login and revocation preserve the guest cookie and history owner; identities are never merged by email. Social login, automatic account linking and public signup are disabled in this staging configuration.

Everything provisioned this session belongs to the existing `aegyo-arcade` Railway project (`8229f87c-908d-426d-9562-4b01b0e89a50`), which Railway lists under **Mateo Daza's Projects**. No workspace or account was moved. The new `accounts-staging` environment is `279e0a09-8ba3-42dc-8d44-a2598d1f3fe9`; original production services and databases were not modified.

| Component                  | Staging resource and state                                                                                                                                                                 |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Accounts                   | [Account pages](https://aegyo-accounts-accounts-staging.up.railway.app/sign-in); service `38c74ef7-2b0b-4e4d-bc95-f32636274e3a`; current deployment `1a22dee3-44a5-46f0-89ca-f7322b9fb272` |
| Arcade preview             | [Arcade](https://arcade-auth-preview-accounts-staging.up.railway.app); service `01e424b3-6137-4ef6-b8fc-94def7963b48`; browser-tested deployment `42427b71-3db6-4ce2-9d29-1053bbb7486e`    |
| Dedicated staging Postgres | Service `950e29c3-1592-4b09-be95-572ce43030c5`; Accounts schema version 1 and restricted role; separate empty `arcade_auth_staging` logical database and application role for Arcade       |
| External database access   | Temporary operator TCP proxy removed after the proof; zero public proxies remain on this new database. Both services remain healthy over private connections.                              |

One synthetic member, a disabled bootstrap operator and three OAuth clients were seeded. Arcade uses a real preview callback; Aegyo and Daebak remain synthetic protocol clients, not deployed product integrations. Accounts runtime verifies the authenticated database CA and pinned leaf certificate. Operator credentials are absent from the running service. Signup and real email delivery remain disabled; the final `account.aegyoarena.com` hostname is not configured yet.

Railway's real proxy was tested with four authenticated requests, including spoofed forwarding headers. It overwrote the supplied client IPs; only then was the database rate limiter switched to the verified `x-real-ip` mode. The diagnostic endpoint requires a reader key in staging and is absent in production. Arcade's origin check now uses its explicitly configured public origin to handle Railway's internal HTTP URL without trusting arbitrary forwarded origins. Final source-only hardening additionally preserves authority for double-slash paths and rejects query/fragment-bearing origin configuration; those two changes await the next preview deployment.

## Evidence from this execution

- **Real Chromium, two deployed HTTPS origins, seven checks passed** at `2026-09-11T23:06:51.790Z`: guest bootstrap; desktop OAuth with language switch; preserved guest identity; local logout and SSO reuse; fresh mobile login; deployed reset UI; stale provider cookie after clearing only app cookies; old Arcade membership revoked within the bounded freshness window. Related assertions are grouped in seven scenarios. Desktop and 390-pixel mobile screenshots were visually reviewed with no clipping or horizontal overflow.
- The recovery link was generated through the same provider core connected to the synthetic staging database, with an in-memory mail recorder. **Mailjet delivery was not tested.** The actual password update and subsequent sign-ins used the deployed pages and handlers.
- **Linux Node 24.21.0:** 17 unit/runtime/UI tests and 21 serial PostgreSQL provider/migration/seed tests passed, zero skips. This includes credential/operator races, restricted-role operation, migration refusal/drift and seed safety. A separate GitHub Actions workflow now runs the isolated Docker proof with no runtime network or deployment credentials.
- **Arcade: 368 unit tests across 51 files passed under Node 24.21.0.** Typecheck, repository-wide formatting and the production build passed. ESLint exited successfully with five Next-specific navigation warnings in the standalone Accounts JavaScript, which does not use Next routing. Root PostgreSQL integration tests still require their separate database fixture; the unit pass does not claim those passed. Generated migration metadata was formatted without changing its data.
- Root Next and eslint-config-next were patched to 16.3.5 for the [existing image-optimization advisory](https://github.com/vercel/next.js/security/advisories/GHSA-2xp9-vwfh-vxw4). Production dependency audit found zero vulnerabilities; four moderate development-tool findings remain through Drizzle's esbuild tooling.
- Root pnpm 9 lockfile was reconciled and a clean frozen install passed. The temporary Railway preview currently uses `npm ci`, so its matching root npm lock remains. The repository's authoritative pnpm workflow should be used for the future Git-connected deployment; synchronize or remove the temporary npm path before changing dependencies. The independent Accounts service intentionally retains its own npm lock and exact Node pin.

Private browser/proxy results, screenshots, synthetic credentials and infrastructure configuration remain ignored under `services/accounts/.proof`, with private permissions on credential/evidence files. None belong in Git or review output. The browser proof intentionally changes only the seeded synthetic password. Its recovery-issuance step needs a separately reviewed temporary connection again now that the test proxy has been removed; do not leave public database access open for convenience.

## Next work and release dependencies

1. **Simon:** obtain read-only Aegyo inventory access, a restorable staging snapshot and private confirmation/transfer of the effective legacy password pepper. Keep production local IDs, roles, sessions and every related record intact; rehearse the copy/freeze/rehash/rollback before cutover.
2. **Email and domain:** configure a verified Mailjet sender and scoped credentials, prove actual recovery/verification delivery, then configure the approved Accounts hostname. Mateo was asked where the Mailjet configuration is held; no secrets should be pasted into chat.
3. **Two remaining adapters:** connect Aegyo while preserving its existing local session consumers; connect Daebak with link-before-provision for every existing Privy identity, wallet and grant. Privy production entitlement/custom-auth access and the fallback's fresh-browser UX still need proof. Dashboard access alone is not entitlement.
4. **Complete G1:** all three real browser origins, password reset and operator revocation, global/RP/back-channel logout behavior, local session renewal, active-game preservation, actual legacy migration/rehash, backup/restore and rollback. Accounts-only logout and Arcade local logout are not proof of global logout.
5. **After shared-auth acceptance:** roll out the account foundation, then implement verified competition attempts, points, usernames and monthly standings. No leaderboard or chat code was added in this execution.

The September 12 configuration/access checkpoint and September 14 three-product proof checkpoint remain conditional on the above dependencies; September 18 is not a guaranteed production date. There is no scheduled follow-up. Execution authorization persists, but this staging evidence does not authorize an unreviewed real-user migration.

## Earlier audit and proof history

The remaining sections preserve earlier checkpoints, including the abandoned Auth0 work. Statements such as “local-only,” “not deployed,” or “provider configuration missing” below describe those earlier checkpoints; the current state and next work are the sections above.

## Historical record: initial Better Auth proof — September 11
...
1. The service, pages and Arcade adapter are now deployed in isolated staging, as recorded above. Complete the two remaining product adapters, safe legacy migration/rehash, and all-three acceptance before production.
