...
required AEGYO_REHEARSAL_SCHEMA_STATUS
...
case "$phase" in
 ...
 resume-inspect) for n in REHEARSAL_LEGACY_OWNER_DATABASE_URL REHEARSAL_DATABASE_CA_CERT REHEARSAL_DATABASE_SERVER_SHA256 ACCOUNTS_REAL_EXPECTED_SESSIONS; do required "$n"; done ;;
...
# A legacy snapshot needs the additive schema. A current additive snapshot must
# already contain empty mapping/latch tables and is never migrated twice.
...
if [ "$AEGYO_REHEARSAL_SCHEMA_STATUS" = legacy ]; then
  ...
  echo 'additive_schema_already_present=true' >"$work/additive.log"
fi
...
node -e 'const x=require(process.argv[1]);if(x.schemaStatus!=="additive-v1"||x.evidenceVersion!==2||x.sharedAuthMappings!==Number(process.env.ACCOUNTS_IMPORT_EXPECTED_COUNT)||x.cutoverLatches!==1)process.exit(2)' "$work/local-after-actual.json" || fail post_mapping_snapshot_schema_or_evidence_invalid
