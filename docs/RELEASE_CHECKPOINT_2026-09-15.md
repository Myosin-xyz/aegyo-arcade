...
Daebak production's seven tracked migration hashes match its Drizzle journal exactly. The binding table is absent. Aggregate counts remain 15 users, 23 grants, four referral codes and three attributions. A fresh backup and explicit standalone binding-table installation are still required before enabling its integration; the current production runtime uses the existing broad database-owner role.
