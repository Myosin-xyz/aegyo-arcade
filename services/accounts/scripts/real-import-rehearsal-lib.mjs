...
export function validateRealImportRehearsal(input) {
  if (input.confirm !== "restored-data-accounts-import-reconciliation-rehearsal")
  ...
  if (!/^aegyo_auth_rehearsal_[0-9]{8}_[a-z0-9]{6,16}$/.test(input.legacyDatabase))
    refuse("legacy_database_not_new_rehearsal_target");
  if (!/^accounts_rehearsal_[0-9]{8}_[a-z0-9]{6,16}$/.test(input.accountsDatabase))
  ...
  for (const [key, value] of Object.entries(input.urls ?? {})) {
    ...
    if (parsed.protocol !== "postgresql:" || !parsed.hostname.endsWith(".railway.internal") || parsed.search)
      refuse(`${key}_must_be_query_free_private_railway_url`);
  }
  if (Object.keys(input.urls ?? {}).length !== 3) refuse("three_database_connections_required");
  return true;
}
