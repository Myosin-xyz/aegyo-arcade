...
try {
  if (
    ...
    process.env.ACCOUNTS_PRODUCTION_CLIENT_CONFIRM !== "offline-first-party-clients"
  ...
  const environment = { ...process.env, ACCOUNTS_PRODUCTION_CLIENT_MANIFEST: path };
  delete environment.ACCOUNTS_PRODUCTION_CLIENTS_JSON;

  // Child output stays private, including exceptional provider diagnostics.
  await execute(process.execPath, [new URL("./provision-production-clients.mjs", import.meta.url).pathname], {
    env: environment, timeout: 120_000, maxBuffer: 65_536,
  });
  const { stdout } = await execute(process.execPath, [new URL("./verify-production-preparation.mjs", import.meta.url).pathname], {
    env: { ...environment, ACCOUNTS_EXPECTED_CLIENTS: "3" }, timeout: 45_000, maxBuffer: 65_536,
  });
  const result = JSON.parse(stdout.trim().split("\n").at(-1));
  if (result.databaseVerified !== true || result.clients !== 3 || result.users !== 0 || result.authenticationActivated !== false)
    throw new Error("preparation_verification_failed");
  await report(process.stdout, result);
} catch {
  await report(process.stderr, { productionClientPreparation: "failed", authenticationActivated: false });
  process.exitCode = 1;
} finally {
