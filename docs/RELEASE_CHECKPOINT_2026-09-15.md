...
1. **Accounts hostname:** authoritative DNS still lacks `account.aegyoarena.com`. Use the final DNS-only CNAME from the accepted Accounts placement below; `2tmkqmk3.up.railway.app` belonged to the old empty placement and is superseded. Resend DNS is a different set of records and is complete.
2. **Final production migration:** the restored-data rehearsal verified 54 imported users, 26 retained legacy sessions and the original 48-table restore. Corrected linked-record ownership evidence is still required after the collector defect described below. Production also requires a fresh coordinated credential-writer freeze, snapshot, import, mapping reconciliation and activation; a successful clone does not migrate the live products.
...
Production Accounts is being relocated into the existing Myosin Aegyo project/environment, with a separate dedicated PostgreSQL service and an independent deployment from the Arcade repository. This preserves the maintained import path without adding a custom encrypted-export protocol. Read-only preflight confirmed the old target has zero users/accounts/sessions/tokens and exactly three OAuth clients. Preserve those exact client IDs/secrets, provider secret and reader keys. Do not delete the old volume or transfer user data during provisioning. Keep both traffic and signup disabled until the final freeze/import/continuity gates pass. The final DNS target must come from the accepted new service; the old CNAME target is no longer the intended final setup.
