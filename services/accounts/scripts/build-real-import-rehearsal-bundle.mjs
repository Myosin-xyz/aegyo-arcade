...
if (!aegyoInput || !outputInput) throw new Error("usage: AEGYO_CHECKOUT PRIVATE_OUTPUT_DIR [ACCOUNTS_REF]");
const accounts = await realpath(new URL("../../..", import.meta.url).pathname);
const aegyo = await realpath(aegyoInput);
const output = resolve(outputInput);
const outputRelative = relative(join(accounts, "services/accounts/.proof"), output);
if (outputRelative.startsWith("..") || outputRelative.startsWith("/")) throw new Error("bundle_output_must_be_under_accounts_proof");
const git = (cwd, args) => execFileSync("git", args, { cwd, encoding: "utf8" }).trim();
const aegyoRef = "e9e0468fe7dc477e3188d18d0ab769d5bddb990e";
if (git(aegyo, ["rev-parse", "HEAD"]) !== aegyoRef) throw new Error("unexpected_aegyo_commit");
...
async function copyTracked(repo, ref, source, destination) {
  const bytes = execFileSync("git", ["show", `${ref}:${source}`], { cwd: repo });
  const path = join(output, destination);
  await mkdir(dirname(path), { recursive: true, mode: 0o700 });
  const file = await open(path, "wx", 0o600);
  try { await file.write(bytes); } finally { await file.close(); }
}
const accountFiles = git(accounts, ["ls-tree", "-r", "--name-only", accountCommit, "services/accounts"])
  .split("\n").filter((path) =>
    ...
    ["check-runtime.mjs", "import-legacy.mjs", "migrate.mjs", "real-import-rehearsal-lib.mjs", "validate-real-import-rehearsal.mjs", "verify-real-canary-signin.mjs"].some((name) => path === `services/accounts/scripts/${name}`));
...
const aegyoFiles = [
  ...
  "scripts/shared-auth/reconcile.mjs", "scripts/shared-auth/reconciliation-lib.mjs",
  "scripts/shared-auth/install-mappings.mjs", "scripts/shared-auth/mapping-installer-lib.mjs",
  "scripts/shared-auth/real-restore-operator.sh",
];
for (const path of aegyoFiles) await copyTracked(aegyo, aegyoRef, path, `aegyo/${path}`);
await copyTracked(accounts, accountCommit, "services/accounts/operator-aegyo/package.json", "aegyo/package.json");
await copyTracked(accounts, accountCommit, "services/accounts/operator-aegyo/package-lock.json", "aegyo/package-lock.json");
await copyTracked(accounts, accountCommit, "services/accounts/scripts/real-import-rehearsal-operator.sh", "operator.sh");
...
try { await file.write(dockerfile); } finally { await file.close(); }
console.info(JSON.stringify({ accountCommit, aegyoCommit: aegyoRef, files: accountFiles.length + aegyoFiles.length + 4 }));
