...
const required = (name) => process.env[name] || (() => { throw new RehearsalRefusal(`missing_${name}`); })();
...
try {
  ...
  pool = new pg.Pool({
    ...databaseOptions(config.sourceUrl, { caCertificate: config.caCertificate }),
    max: 1,
  ...
  if (identity.read_only !== "on" || identity.rolsuper || identity.rolcreatedb || identity.rolcreaterole || !identity.durable_read_only)
  ...
  const inventory = (await client.query(`SELECT
    ...
    (SELECT count(*)::int FROM public."Session") AS "sessionCount"`)).rows[0];
  assertInventory(inventory, { tableCount: config.expectedTables, userCount: config.expectedUsers, sessionCount: config.expectedSessions });
  ...
  if (canaryProvided) {
    ...
    if (rel.startsWith("..") || rel.startsWith("/")) throw new RehearsalRefusal("credential_output_outside_private_proof_directory");
    const file = await open(output, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, 0o600);
    try {
      await file.writeFile(JSON.stringify({ sourceUserId: config.canarySourceUserId, password: config.canaryPassword, deployedPepperDigest: config.deployedPepperDigest }) + "\n");
      await file.sync();
    } finally { await file.close(); }
    canaryVerified = true;
  }
  await client.query("ROLLBACK");
  console.info(JSON.stringify({ databaseVerified: true, durableReadOnly: true, ...inventory, snapshotDigest: captured.snapshotDigest, canaryRequired: !canaryVerified, canaryVerified }));
} catch (error) {
  console.error(error instanceof RehearsalRefusal || error instanceof ImportRefusal ? error.message : "real_rehearsal_preflight_failed");
  process.exitCode = 1;
} finally {
